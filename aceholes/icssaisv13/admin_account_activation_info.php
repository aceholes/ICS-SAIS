...
<html>
	...
	<body>
		<form name="admin_account_activation_info" action="admin_account_activation_redirect.php" method="post"/>
			<!-- top right po	-->
			...
			<?php
				if(isset($_GET['stdnum'])){
					...
					if($conn){
						$result = odbc_exec($conn, "select sno, sfname, slname, sgender, shaddr, scaddr, shcontact, sccontact, smobile, suser, semail, sregdate, scurr, sspgrad, sappr, sact from student where sno = '".$_GET["stdnum"]."'" );

						echo "<table border='1'>";
						while($row = odbc_fetch_array($result)) {
							$_SESSION["account_activation_number"]=$row["SNO"];
							...
							echo "</tr>";
							echo "<tr>";
								echo "<td>SP GRADUATE</td>";
								echo "<td>".$row["SSPGRAD"]."</td>";
							echo "</tr>";
							echo "<tr>";
								echo "<td>APPROVAL</td>";
								echo "<td>".$row["SAPPR"]."</td>";
							echo "</tr>";
							echo "<tr>";
					...
					if($conn){
						...
						echo "<table border='1'>";
						while($row = odbc_fetch_array($result)) {
							$_SESSION["account_activation_number"]=$row["INO"];
				...
				}else if(isset($_GET['encnum'])){
			
					$conn = odbc_connect("Driver={Microsoft ODBC for Oracle};Server=localhost;Uid=tantan;Pwd=tantan;", 'tantan', 'tantan');

					if($conn){
						$result = odbc_exec($conn, "select eno, efname, elname, egender, ehaddr, ecaddr, ehcontact, eccontact, emobile, euser, eemail, eregdate, erole, eact from encoder where eno = '".$_GET["encnum"]."'" );

						echo "<table border='1'>";
						while($row = odbc_fetch_array($result)) {
							$_SESSION["account_activation_number"]=$row["ENO"];
							...
							echo "<tr>";
								echo "<td>REGISTRATION DATE</td>";
								echo "<td>".$row["EREGDATE"]."</td>";
							echo "</tr>";
							echo "<tr>";
								echo "<td>ROLE</td>";
								echo "<td>".$row["EROLE"]."</td>";
							echo "</tr>";
							echo "<tr>";
								echo "<td>ACTIVATION</td>";
								echo "<td>".$row["EACT"]."</td>";
			...
			<input type="submit" name="admin_account_activation_submit" value="Submit"/><br/><br/>
