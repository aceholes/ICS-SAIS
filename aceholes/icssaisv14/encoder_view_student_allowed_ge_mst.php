...
<html>
	...
	<body>
		<form name="encoder_view_student_allowed_ge_mst" action="encoder_modify_student_allowed_redirect.php" method="post">
			<!-- top right po	-->
			...
			<b>MST</b> &nbsp;&nbsp;&nbsp;
			<a href="encoder_view_student_allowed_ge_ssp.php"/>SSP</a><br/><br/>
					...
					if($flag != 0){
						...
						while($row2 = odbc_fetch_array($result2)) {
							echo "<tr>";
								...
								echo "<td><a name='geno_mst' href='encoder_modify_student_allowed_redirect.php?geno_mst=".$row2["GENO"]."'>Delete</a></td>";
							echo "</tr>";						
						}
						...
						$result3 = odbc_exec($conn, "select geno, getitle from ge where gedomain='MST' and geno not in (select geno from ge_allowed)");
						...
						if($flag2 != 0){
							$result4 = odbc_exec($conn, "select geno, getitle from ge where gedomain='MST' and geno not in (select geno from ge_allowed)");

							echo "<select name='encoder_add_student_allowed_ge_mst_no' required='required'>";
							echo "<option></option>";
							...
							echo "<input type='submit' name='encoder_add_student_allowed_ge_mst_submit' value='Add Subject'/>";
