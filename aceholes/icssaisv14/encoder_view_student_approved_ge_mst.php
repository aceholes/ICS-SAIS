...
<html>
	...
	<body>
		<form name="encoder_view_student_approved_ge_mst" action="encoder_modify_student_approved_redirect.php" method="post">
			<!-- top right po	-->
			...
			<a href="encoder_view_student_approved_ge_ah.php"/>AH</a> &nbsp;&nbsp;&nbsp;
			<b>MST</b> &nbsp;&nbsp;&nbsp;
			...
			<?php
				...
				if($conn){
					...
					$result = odbc_exec($conn, "select b.geno, b.getitle, b.gedesc, b.geunits
						 						...
						 						b.gedomain='MST' and a.sno='".$_SESSION["sno"]."'");
					...
					if($flag != 0){
						...
						while($row2 = odbc_fetch_array($result2)) {
							echo "<tr>";
								...
								echo "<td><a name='geno_mst' href='encoder_modify_student_approved_redirect.php?geno_mst=".$row2["GENO"]."'>Delete</a></td>";
							echo "</tr>";						
						}
					...
					if($count < 5){
						$result4 = odbc_exec($conn, "select geno, getitle from ge where gedomain='MST' and geno not in (select geno from ge_approved)");
						...
						if($flag2 != 0){
							echo "Add Subject<br/>";
							echo "<select name='encoder_add_student_approved_ge_mst_no' required='required'>";
							echo "<option></option>";

							$result5 = odbc_exec($conn, "select geno, getitle from ge where gedomain='MST' and geno not in (select geno from ge_approved)");
							...
							echo "</select><br/>";
							echo "Semester";
							echo "<input type='radio' name='encoder_add_student_approved_ge_mst_sem' value='1st' required='required'/>1st";
							echo "<input type='radio' name='encoder_add_student_approved_ge_mst_sem' value='2nd' required='required'/>2nd";
							...
							$ay="";
							echo "<br/>Year";
							echo "<select name='encoder_add_student_approved_ge_mst_year' required='required'>";
								echo "<option></option>";
								for($i = 0; $i < $gap; $i++){
									$until = $currentyear + 1;
							...
							echo "<input type='submit' name='encoder_add_student_approved_ge_mst_submit' value='Submit'/><br/>";
