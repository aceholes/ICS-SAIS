...
<html>
	...
	<body>
		<form name="encoder_view_student_approved_ge_ssp" action="encoder_modify_student_approved_redirect.php" method="post">
			<!-- top right po	-->
			...
			<?php
				...
				if($conn){
					...
					if($count < 5){
						$result4 = odbc_exec($conn, "select geno, getitle from ge where gedomain='SSP' and geno not in (select geno from ge_approved)");
						...
						if($flag2 != 0){
							echo "Add Subject<br/>";
							echo "<select name='encoder_add_student_approved_ge_ssp_no' required='required'>";
							echo "<option></option>";

							$result5 = odbc_exec($conn, "select geno, getitle from ge where gedomain='SSP' and geno not in (select geno from ge_approved)");
							...
							echo "</select><br/>";
							echo "Semester";
							echo "<input type='radio' name='encoder_add_student_approved_ge_ssp_sem' value='1st' required='required'/>1st";
							echo "<input type='radio' name='encoder_add_student_approved_ge_ssp_sem' value='2nd' required='required'/>2nd";
							...
							$ay="";
							echo "<br/>Year";
							echo "<select name='encoder_add_student_approved_ge_ssp_year' required='required'>";
								echo "<option></option>";
								for($i = 0; $i < $gap; $i++){
									$until = $currentyear + 1;
							...
							echo "<input type='submit' name='encoder_add_student_approved_ge_ssp_submit' value='Submit'/><br/>";
