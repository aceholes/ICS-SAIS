...
<?php
	...
	if(isset($_POST["instructor_change_password_submit"])){
		...
		if($conn){
			if($old!=""){
				$result=odbc_exec($conn, "select ipword from instructor where ino = '".($_SESSION['no'])."'");
			...
			if($new!="" && $new2!="" && $_SESSION["password_not_found"]==0){
				if($new!=$new2){
					...
					odbc_exec($conn, "update instructor set ipword = '".($new)."' where ino = '".($_SESSION['no'])."'");

					odbc_autocommit($conn, TRUE);
					odbc_exec($conn, "insert into instructor_log values ('".($_SESSION["no"])."','".($_SESSION['fname'])." ".($_SESSION['lname'])." changes password."."',sysdate)");
