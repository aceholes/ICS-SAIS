...
<html>
	...
	<body>
		<form name="encoder_view_student_allowed_ge_ah" action="encoder_modify_student_allowed_redirect.php" method="post">
			<!-- top right po	-->
					...
					if($flag != 0){
						...
						while($row2 = odbc_fetch_array($result2)) {
							echo "<tr>";
								...
								echo "<td><a name='geno_ah' href='encoder_modify_student_allowed_redirect.php?geno_ah=".$row2["GENO"]."'>Delete</a></td>";
							echo "</tr>";						
						}
						...
						$result3 = odbc_exec($conn, "select geno, getitle from ge where gedomain='AH' and geno not in (select geno from ge_allowed)");
						...
						if($flag2 != 0){
							$result4 = odbc_exec($conn, "select geno, getitle from ge where gedomain='AH' and geno not in (select geno from ge_allowed)");

							echo "<select name='encoder_add_student_allowed_ge_ah_no' required='required'>";
							echo "<option></option>";
							...
							echo "<input type='submit' name='encoder_add_student_allowed_ge_ah_submit' value='Add Subject'/>";
