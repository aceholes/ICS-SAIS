	...
	<body>
		<form name="admin_request_approval"/>
			...
			<?php
				...
				if($conn){
					$result = odbc_exec($conn, "select slname, sfname, sno from student where sappr='Pending' and sact='Deactivated' ");
			...
			<?php
				...
				if($conn){
					$result = odbc_exec($conn, "select ilname, ifname, ino from instructor where iappr='Pending' and iact='Deactivated' ");
