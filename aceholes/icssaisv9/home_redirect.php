			...
			if($_SESSION["signup_number_invalid"]==1 || $_SESSION["signup_firstname_invalid"]==1 || $_SESSION["signup_lastname_invalid"]==1 || $_SESSION["signup_emailaddress_invalid"]==1 || $_SESSION["signup_username_invalid"]==1 || $_SESSION["signup_password_match_error"]==1){
				...
				if($conn){
					if($role=="student"){
						...
						odbc_autocommit($conn, TRUE);
						odbc_exec($conn, "insert into instructor (ino, ifname, ilname, igender, iuser, ipword, iemail) values ('".htmlspecialchars($number)."','".htmlspecialchars($firstname)."','".htmlspecialchars($lastname)."','".htmlspecialchars($gender)."','".htmlspecialchars($username)."','".htmlspecialchars($password)."','".htmlspecialchars($emailaddress)."')");	
					}
					$_SESSION["no"]=$number;
