		...
		<form name="admin_view_users_info" action="admin_delete_user_redirect.php" method="post"/>
			...
			<?php
				if(isset($_GET['stdnum'])){
					...
					if($conn){
						$result = odbc_exec($conn, "select sno, sfname, slname, sgender, shaddr, scaddr, shcontact, sccontact, smobile, suser, spword, semail, sregdate, scurr, sclass, sappr, sact from student where sno = '".$_GET["stdnum"]."'" );
					...
					if($conn){
						$result = odbc_exec($conn, "select ino, ifname, ilname, igender, ihaddr, icaddr, ihcontact, iccontact, imobile, iuser, ipword, iemail, iregdate, idesig, irank, iroom, iappr, iact from instructor where ino = '".$_GET["insnum"]."'" );
