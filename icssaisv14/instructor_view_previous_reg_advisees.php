...
<html>
	...
	<body>
		<form name="instructor_view_previous_reg_advisees"/>
			...
			<h3>PREVIOUS REGISTRATION ADVISEES HERE!</h3>
			...
			<?php
				...
				if($conn){
					...
					$result = odbc_exec($conn, "select a.sno, a.sfname, a.slname, b.prastartdate, b.praenddate from student a, prev_reg_advisees b 
					...
					if($flag != 0){
						$result2 = odbc_exec($conn, "select a.sno, a.sfname, a.slname, b.prastartdate, b.praenddate from student a, prev_reg_advisees b 
												where a.sfname = (select sfname from student where sno=b.sno) and
												a.slname = (select slname from student where sno=b.sno)
												and a.sno = b.sno and ino = '".$_SESSION["no"]."'");
