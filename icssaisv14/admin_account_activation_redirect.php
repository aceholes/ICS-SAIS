		...
		if($conn){
			if($_SESSION["role"]=="student"){
				...
				if($action=="---"){
					...
					odbc_autocommit($conn, TRUE);
					odbc_exec($conn, "update encoder set eact='Activated' where eno = '".$_SESSION["account_activation_number"]."'" );	
					$_SESSION["activated"]=1;
					header("Location: admin_account_activation.php");
					exit;
				}else if($action=="deactivate"){
					odbc_autocommit($conn, TRUE);
					odbc_exec($conn, "update encoder set eact='Deactivated' where eno = '".$_SESSION["account_activation_number"]."'" );	
					$_SESSION["deactivated"]=1;
					header("Location: admin_account_activation.php");
					exit;
				}
