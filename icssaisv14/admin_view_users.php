	...
	<body>
		<form name="admin_view_users" action="admin_view_users_info.php" method="post"/>
			...
			<?php
				...
				if($conn){
					$result = odbc_exec($conn, "select elname, efname, eno from encoder");

					echo "<table>";
					while($row = odbc_fetch_array($result)) {
						echo "<tr>";
							echo "<td>".$i++.". </td>";
							$name=$row["ELNAME"].", ".$row["EFNAME"];
							$num=$row["ENO"];
							echo "<td><a name='encnum' href='admin_view_users_info.php?encnum=".$num."'>$name</a></td>";
						echo "</tr>";
					}
					echo "</table><br/><br/>";	
